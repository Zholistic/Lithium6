function [logSigmaY,logNROISum] = TopCamN_CrossoverCalc(directory,datestring)
%-------------------------------------------------------------------------%
...
%Full region
ROIy = 1:length(imgArrayFresh(:,1,1));
ROIx = 1:length(imgArrayFresh(1,:,1));
...
if(bin ~= 1)
    ...
    %Atoms number for ROISum on each individual image.
    NROISum = [];
...
if(bin)
    ...
    for i=1:length(imgArrayFresh(1,1,:))
        ...
        [coefs,resnorm,r,exitflag,output,lambda,J] = lsqcurvefit(fg,p0,xsY(:),profArray(:,i),lb,ub,curvefitoptions);
    ...
    profArray = imgXs; %Then X profiles
    ...
    for i=1:length(imgArrayFresh(1,1,:))
        ...
        curvefitoptions = optimset('MaxFunEvals',100000,'MaxIter',50000,'Display','off');
        xsX = 1:length(profArray(:,i));
        [coefs,resnorm,r,exitflag,output,lambda,J] = lsqcurvefit(fg,p0,xsX(:),profArray(:,i),lb,ub,curvefitoptions);
        %[coefs,resnorm,r,exitflag,output,lambda,J] = lsqcurvefit(fg,p0,xs(:),profArray(:,7),lb,ub,curvefitoptions);
...
if(fittingGaus)
    ...
    %Fit to Y or X:
    profArray = imgYm; %First Y profiles
    ...
    for i=1:length(imgArray(1,1,:))
        ...
        [coefs,resnorm,r,exitflag,output,lambda,J] = lsqcurvefit(fg,p0,xs(:),profArray(:,i),lb,ub,curvefitoptions);
        %[coefs,resnorm,r,exitflag,output,lambda,J] = lsqcurvefit(fg,p0,xs(:),profArray(:,7),lb,ub,curvefitoptions);
    ...
    profArray = imgXm; %Then X profiles
...
if(fittingGaus) %Skip this
    ...
    for i=1:length(imgArray(1,1,:))
        if(mod(i,1) == 0) %Only disp every one...

...
close all;
open('C:\Users\tpeppler\Dropbox\PhD\2D_2016\EOS_Data\972_scatterGraph3D_Iso_AvgClouds.fig');
handles(1) = gcf;
open('C:\Users\tpeppler\Dropbox\PhD\2D_2016\EOS_Data\920_scatterGraph3D_Iso_AvgClouds.fig');
handles(2) = gcf;
open('C:\Users\tpeppler\Dropbox\PhD\2D_2016\EOS_Data\880_scatterGraph3D_Iso_AvgClouds.fig');
handles(3) = gcf;
open('C:\Users\tpeppler\Dropbox\PhD\2D_2016\EOS_Data\865_scatterGraph3D_Iso_AvgClouds.fig');
handles(4) = gcf;
...
xlin = linspace(min(x)-1,max(x)+1,60);
ylin = linspace(min(y)-1,max(y)+1,60);
        

[X,Y] = meshgrid(xlin,ylin);

%Z = griddata(x,y,z,X,Y,'cubic');
Z = griddata(x,y,z,X,Y,'natural'); %v4 cubic linear
...
figname = ['basic_surface_Kdata_interpOnBinnedDataFromScatter_AvgClouds'];

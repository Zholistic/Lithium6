...
%plot(fg(gfitcoefsYF(:,5),xsY),'r'); hold on; plot(imgYs(:,5),'b');
profArrayX = [];
...
if(sidecam)
    ...
    for i=1:length(imgArray(1,1,:))
        if(mod(i,dispNumber) == 0) %Only disp every one...
            ...
            subplot(2,1,1);imagesc(imgArray(:,:,i));%,'CDataMapping','scaled'); caxis ([-1 15]);
            subplot(2,1,2); hold on; plot(imgXs(:,i));

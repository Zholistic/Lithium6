directory = 'C:\Data\160518_CollectiveOscillations_832G_DriveFreq_41p1Hz_20cycles_40katoms_driving_feshbachcoils_2p5Vpp_59p2mvpp\';
...
%------------ Gaussian Fit Radius Code ------------%
    ...
    [gcoefsFreqFit,gcoefsFreqFitError] = sinExpDampFitFreqGuess(widthsR,motFets(1:end),0.32);
...
%Post image-avg radial widths:
    ...
    [gcoefsFreqFit,gcoefsFreqFitError] = sinExpDampFitFreqGuess(widthsRpa,motFets(1:end),0.32);
    fgsineDamp = @(p,x)(p(1).*exp(-p(2).*x).*sin(p(3).*x+p(4))+p(5));
    hold on;
    plot(fgsineDamp(gcoefsFreqFit,1:85),'r');
    widthsRfitFreq = gcoefsFreqFit(3)/(10^(-3))/(2*pi);

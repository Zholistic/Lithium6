...
fringeRem = 1;
%Pull images:
%[beamArray, atom1Array, atom2Array] = PullSPEBatch(fileLocList(1:200),Isat,OD,fringeRem);
...
%EOSGenerate(radProfilesAvg(1,:,1)./(kpixelLength^2),radProfilesAvg(2,:,1),2:65,omegaRVector(4))
...
for i=1:length(imageArrayC(1,1,:))
    ...
    radProfilesIndvToEOS(1,:,i) = meanNelements(radProfilesToReorder(1,:,i),250);
    radProfilesIndvToEOS(2,:,i) = meanNelements(radProfilesToReorder(2,:,i),250);
end
...
if(0)   
    calcRegion = 3:45;
    for i=1:length(radProfilesIndvToEOS(1,1,1:200))
        if(mod(i,10) == 0)
            figure(i);
            plot(radProfilesIndvToEOS(2,calcRegion,i),radProfilesIndvToEOS(1,calcRegion,i),'r'); grid on; hold off;
        end
    end
end
...
if(0)
   ...
   for i=1:length(radProfilesIndvToEOS(1,1,:))
       radProfSmoothed(:,i) = smooth(radProfilesIndvToEOS(1,:,i));
   end               
end
...
shiftByZ = [];
for i=1:length(radProfilesIndvToEOS(1,1,1:200))
    shiftByZ(i) = mean(radProfilesIndvToEOS(1,54:80,i));
    radProfilesIndvToEOS(1,:,i) = radProfilesIndvToEOS(1,:,i) - shiftByZ(i);
...
%EOSGenerateBulk(radProfilesAvg(1,:,1)./(kpixelLength^2),radProfilesAvg(2,:,1),2:65,omegaR,972,a2d,smoothOn,saveOn,savestring)

calcRegion = 2:52;
omegaZ = 5150*2*pi;
%fitVirial(radProfilesAvg(1,:,1)./(kpixelLength^2), radProfilesAvg(2,:,1),calcRegion, omegaZ, omegaR, 972, a2d, smoothOn, zeroOn)

%Script to test the new functions:
...
X1BD = (-1)*((hbar^2) / massL6).*gradient(radialDensity(1,:),radialDensity(2,:));
...
for i=1:(length(radialDensity(1,:)))
  
    Xm1BD(i) = (massL6 / (hbar^2))*(1/(radialDensity(1,i)^2)).*trapz(radialDensity(2,i:end),radialDensity(1,i:end),2);
...
nbins = 50; binMean = []; radialDensity = []; binStd = []; binEdges=[];

...
directory = 'C:\Data\EOS_Data\140913_2d_eos_865g_1p2v_evap_1sec_2d_ramp\';
date = '140913';
camera = 'top';
varstring = 'isat';
%varstring2 = 'Holdtime';
...
for i=1:length(radProfiles(1,1,:))
    shiftBy(i) = mean(radProfiles(1,60:74,i));
...
%EOSGenerate(radProfilesAvg(1,:,1)./(kpixelLength^2),radProfilesAvg(2,:,1),2:65,omegaRVector(1))
...
a2d = a2dVectorFiveOneFive(1);
omegaR = omegaRVector(1);
saveOn = 0;
smoothOn = 1;
zeroOn = 1;
savestring = 'indv clouds smoothed';
%EOSGenerateBulk(radProfilesIndvToEOS(1,:,1:200)./(kpixelLength^2),radProfilesIndvToEOS(2,:,1:200),calcRegion,omegaR,865,a2d,smoothOn,zeroOn,saveOn,savestring)
...
cutoffLeft = 23;
cutoffRight = 0;
%fitVirial(radProfilesAvg(1,:,1)./(kpixelLength^2), radProfilesAvg(2,:,1),calcRegion, omegaZ, omegaR, 865, a2d, smoothOn, zeroOn,cutoffLeft,cutoffRight)
...
potential = 0.5 .* massL6 .* omegaRVector(1)^2 .* radiusVectorMeters.^2;
potential_nk = (potential./kB).*(10^9);
%plot(potential_nk,radProfilesAvg(1,:,i));

%convert radProfile pixels to potential:
radProfilesPotential = 0.5 .* massL6 .* omegaRVector(1)^2 .*(radProfilesAvg(2,:,1).*pixelLength).^2;

...
kappaTildeAvg = (-1)*(pi * hbar^2 / massL6) * gradient(radproftoFityReal(calcRegion),potential(calcRegion));


for j=1:length(calcRegion)
    pAvg(j) = trapz(potential(j:calcRegion(end)),radproftoFityReal(j:calcRegion(end))); %integral from end to j point
...
title('kappa vs p');
figure(10); h1 = plot(pTildeAvg,'.');
title('p vs r (pixel)');
figure(11); plot(kappaTildeAvg,'.');
title('kappa vs r (pixel)');
figure(12); plot(potential(calcRegion),radproftoFityReal(calcRegion),'.');
title('density vs potential');

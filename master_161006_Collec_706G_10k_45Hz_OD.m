directory = 'C:\Data\161006_CollectiveOsc_2D_706G_10katom_125mvPtoP_45Hz_5Cycles_13mix_ODcorr\';
...
shiftBy = [];
for i=1:length(radProfiles(1,1,:))
    shiftBy(i) = mean(radProfiles(1,50:70,i));
    radProfiles(1,:,i) = radProfiles(1,:,i) - shiftBy(i);
end
end
...
%plot(radProfilesAvg(2,:,1),radProfilesAvg(1,:,1));
%OD correction: plot(radProfilesAvg(2,1:50,1),radProfilesAvg(1,1:50,2)./radProfilesAvg(1,1:50,1)); grid on;
%Checked OD correction? Yes
%mean(radProfilesAvg(1,10:20,2)./radProfilesAvg(1,10:20,1)) = 1.48;
%mean(varData(26:end,5))./mean(varData(1:25,5)) = 2.02;
radProfilesAvg(1,:,1) = radProfilesAvg(1,:,1).*2.*1.48;
...
%fitVirial(radProfilesAvg(1,:,1)./(kpixelLength^2), radProfilesAvg(2,:,1),calcRegion, omegaZ, omegaR, 706, a2d, smoothOn, zeroOn,0,0);
